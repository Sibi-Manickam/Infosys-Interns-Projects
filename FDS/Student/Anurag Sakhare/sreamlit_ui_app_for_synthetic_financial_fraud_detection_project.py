# -*- coding: utf-8 -*-
"""Sreamlit UI App for Synthetic Financial Fraud Detection Project.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1LYBhqL8MWAyX2RSAK3zhtoRGnmnRWD_s
"""
...
def preprocess_data(df):
    ...
    y = df['isFraud'].astype(int)  # Ensure y is binary

    # Perform one-hot encoding for categorical variables
    X = pd.get_dummies(X, columns=['type', 'branch', 'Acct type', 'Time of day'], drop_first=True)

    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=1, stratify=y)
    sm = SMOTE(random_state=42)
    X_res, y_res = sm.fit_resample(X_train, y_train)
    return X_res, y_res, X_test, y_test, X.columns
